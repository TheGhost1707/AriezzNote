...
<body>
    ...
    <div class="novel-container mt-5">
        ...
        if (isset($_GET['success'])) {
            echo "<div class='alert alert-success' id='successAlert'>" . htmlspecialchars($_GET['success']) . "</div>";
        }
        ?>
                ...
                <div class="card novel-card shadow mb-4">
                    <div class="card-body">
                        ...
                        <?php foreach ($chapters as $key => $chapter) : ?>
                            ...
                            <!-- Tombol Edit dan Hapus untuk setiap Chapter -->
                            <div class="text-end mt-4" id="chapterActions<?= $key ?>" <?php if ($key != 0) echo 'style="display:none;"'; ?>>
                                <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editChapterModal<?= $key ?>">Edit Bab</button>
                                <a href="delete_chapter.php?id=<?= $chapter['id']; ?>&novel_id=<?= $novel_id; ?>" class="btn btn-danger">Hapus Bab</a>
                            </div>
                            ...
                            <!-- Modal for Edit Chapter -->
                            <div class="modal fade" id="editChapterModal<?= $key ?>" tabindex="-1" aria-labelledby="editChapterModalLabel<?= $key ?>" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="editChapterModalLabel<?= $key ?>">Edit Bab <?= htmlspecialchars($chapter['chapter_number']); ?></h5>
                                        ...
                                        <form action="update_chapter.php" method="post">
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    <label for="chapter_title<?= $key ?>" class="form-label">Judul Bab</label>
                                                    <input type="text" class="form-control" id="chapter_title<?= $key ?>" name="chapter_title" value="<?= htmlspecialchars($chapter['chapter_title']); ?>" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="chapter_content<?= $key ?>" class="form-label">Isi Bab</label>
                                                    <textarea class="form-control" id="chapter_content<?= $key ?>" name="chapter_content" required><?= htmlspecialchars($chapter['chapter_content']); ?></textarea>
                                                </div>
                                            </div>
    ...
    <script>
        ...
        <?php foreach ($chapters as $key => $chapter) : ?>
            ...
            document.getElementById('nextBtn<?= $key ?>')?.addEventListener('click', function() {
                ...
                document.getElementById('chapterActions<?= $key ?>').style.display = 'none';
                ...
                document.getElementById('chapterActions<?= $key + 1 ?>').style.display = 'block';
            ...
            document.getElementById('readChapter<?= $key ?>')?.addEventListener('click', function() {
                ...
                <?php foreach ($chapters as $index => $chap) : ?>
                    ...
                    document.getElementById('chapterActions<?= $index ?>').style.display = 'none';
                ...
                document.getElementById('chapterActions<?= $key ?>').style.display = 'block';
                if (document.getElementById('nextBtnDiv<?= $key ?>')) {
    ...
    <script>
        ...
        setTimeout(function() {
            ...
            if (successAlert) {
                ...
                successAlert.style.opacity = '0'; // Hilangkan dengan efek fade out

                // Hapus elemen dari DOM setelah animasi selesai
                setTimeout(function() {
                    successAlert.remove();
                }, 500); // Waktu tambahan untuk memastikan animasi selesai
            }
        }, 3000); // 3 detik
    </script>
</body>
